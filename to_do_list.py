...
class TodoListApp(tk.Tk):
    def __init__(self):
        ...
        #Bind event to restore placeholder when input loses focus
        self.task_input.bind("<FocusOut>",self.restore_placeholder)

        #Create button for adding tasks 
        ttk.Button(self,text="Add",command=self.add_task).pack(pady=5)

        #CreatenListbox to display added tasks
        self.task_list=tk.Listbox(self,font=(
            "TkDefaultFont",16),height=10,selectmode=tk.SINGLE)
        self.task_list.pack(fill=tk.BOTH,expand=True,padx=10,pady=10)

        #Create buttonns for marking tasks as done or deleting them
    ...
    def load_tasks(self):
        try:
            with open("tasks.json","r") as f:
             data=json.load(f)
            for task in data :
                index =self.task_list.insert(tk.END,task["text"])
                self.task_list.itemconfig(index,{'fg':task["color"]})
